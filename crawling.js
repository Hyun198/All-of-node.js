...
async function morningCrawling() {
    try{
        const browser = await puppeteer.launch({headless:true})
        const page = await browser.newPage()
        await page.goto(process.env.cgv)
        ...
        //const cachedFilePath = path.join('cgv','cached_data.txt');
        await fs.writeFile(path.join('cgv','morning_data.txt'),times.join("\r\n"));

        console.log('Morning crawling and caching completed');
    }catch(err){
        console.error('크롤링 중 에러:',err);
    }
}

module.exports = {performCrawling, morningCrawling};